(function() {
  var ProxyServer, crypto, elf, fs, h, http, insert_counter, pg, query_counter, query_error_counter, sys, timing_log, update_counter, upsert_counter, upsert_error_counter, upsert_row_counter, url, util, _;
  sys = require('sys');
  fs = require('fs');
  http = require('http');
  url = require('url');
  ...
  ProxyServer.prototype.handleJSONquery = function(self, client, resp, clientData) {
    ...
    if (!query || !query.table || !query.data) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        _results.push(h.execSqlCount(client, query.table, row, function(err, row, rowCnt) {
          var res, sql;
          if (err) {
            ...
            if (rowCnt > 1) {
              ...
              return res = client.query(sql, function(err, rs) {
                ...
                return timing_log.push((new Date()) - resp.__pg_starttime);
              });
            }
          }
        }));
