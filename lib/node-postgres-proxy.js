(function() {
  ...
  ProxyServer.prototype.handleJSONquery = function(self, client, resp, clientData) {
    ...
    if (!query || !query.table || !query.data) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        (function() {
          var i = _ref[_i];
          return _result.push(h.execSqlCount(client, query.table, query.data[i], function(err, rowCnt) {
            var res, sql;
            if (err) {
  ...
  ProxyServer.prototype.databaseConnection = function(dbName, callback) {
    var _i, _len, _ref, database;
    if (!((function(){ for (var _i=0, _len=(_ref = this.config['databases']).length; _i<_len; _i++) { if (_ref[_i] === dbName) return true; } return false; }).call(this))) {
      return callback({
        status: 404,
