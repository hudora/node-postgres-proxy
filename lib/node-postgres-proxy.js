(function() {
  var ProxyServer, crypto, elf, fs, h, http, insert_counter, pg, query_counter, query_error_counter, sys, timing_log, update_counter, upsert_counter, upsert_error_counter, upsert_row_counter, url, _;
  sys = require('sys');
  fs = require('fs');
  http = require('http');
  url = require('url');
    ...
    server = http.createServer(function(req, resp) {
      ...
      if (url.parse(req.url).pathname === '/stats') {
        ...
        if (inst.hasValidCredentials(req, resp)) {
          ...
          action = parts[0];
          dbName = parts[1];
          if (action === void 0 || dbName === void 0) {
  ...
  ProxyServer.prototype.hasValidCredentials = function(req, resp) {
    var _i, _len, _ref, auth, creds, digest, encoding, hmac, i, secrets;
    var auth, creds, digest, encoding, hmac, secret, secrets, _i, _len;
    digest = req.headers['x-sig'];
    secrets = this.config.secrets;
    if (digest && secrets) {
      for (_i = 0, _len = secrets.length; _i < _len; _i++) {
        secret = secrets[_i];
  ...
  ProxyServer.prototype.handleGET = function(req, resp, action, dbName) {
    ...
    if (clientData === '' || action !== 'sql') {
      return h.sendError(resp, "'sql' Parameter missing on GET request or path does not start with '/sql/'", 400);
    } else {
      return self.databaseConnection(dbName, function(err, client) {
        if (err) {
          return h.sendError(resp, 'problem connectiong to the database: ' + sys.inspect(err), 500);
        } else {
          return self.handleSQLquery(self, client, resp, clientData);
        }
      });
    }
  };
          ...
          switch (action) {
            case 'sql':
              if (!/text-plain/.test(req.headers['content-type'])) {
                return h.sendError(resp, "encode SQL as text/plain", 415);
              } else {
                return self.handleSQLquery(self, client, resp, clientData);
              }
            case 'json':
  ...
  ProxyServer.prototype.handleJSONquery = function(self, client, resp, clientData) {
    var query, row, _i, _len, _ref, _results;
    ...
    if (!query || !query.table || !query.data) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        _results.push(h.execSqlCount(client, query.table, row, function(err, rowCnt) {
          var res, sql;
          if (err) {
            upsert_error_counter = upsert_error_counter + 1;
            return h.sendError(resp, 'Database Error: ' + err, 500);
          } else {
            if (rowCnt > 1) {
              upsert_error_counter = upsert_error_counter + 1;
              return h.sendError(resp, 'More than one row would be affected', 400);
            } else {
              if (rowCnt === 1) {
              ...
              return res = client.query(sql, function(err, rs) {
                ...
                return timing_log.push((new Date()) - req.__pg_starttime);
              });
            }
          }
        }));
